...
// 设定某列的特征个数
// 筛选出“userfeatures”大小不正确的行（把不正确的过滤没了）
...
object VectorSizeHintExample {
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("VectorSizeHintExample").setMaster("local")
    ...
    val dataset = spark.createDataFrame(
      Seq(
        ...
        (0, 18, 1.0, Vectors.dense(0.0, 11.0, 0.5), 1.0),
        (1, 18, 1.0, Vectors.dense(0.0, 12.0), 1.0),
        (2, 18, 1.0, Vectors.dense(0.0, 13.0), 1.0)
      )
